...
bool can(int wide, int left, vector<int>& qtd, int assentosLivres, int pessoas) {
    ...
    if(assentosLivres < pessoas) return false;

    ull x = mix(wide ^ mix(left));

    for(auto el: qtd) x = mix(el ^ mix(x));

//    cout << x << ' ' << wide << ' ' << left << endl; 
    ...
    for(int i = 0; !ans && i < 12; i++) {
       if(left >= i+1 && i < qtd.size() && qtd[i] > 0) {
           ...
           if(left-(i+1) >= 0) {
                ...
                aux = can(wide, left-(i+2), qtd, assentosLivres-(i+(left-i-2 < 0 ? 1 : 2)), pessoas-i-1);
           }
           ans |= aux; 
