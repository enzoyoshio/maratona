...
auto main() -> signed {
  ...
  V<V<int>> dp(n, vector<int>(4, -oo));

  dp[0][0] = v[0];
  dp[0][1] = v[0] * x;

  int ans = max(v[0], v[0]*x);
  for(int i = 1; i < n; i++) {

    // if I did not start multiplying
    // then this is regular Kadane
    dp[i][0] = max(dp[i-1][0] + v[i], v[i]);

    // if I am on a segment that is multiplying
    // need to check what is best
    // to start now or that it already has started
    dp[i][1] = max(dp[i-1][0] + v[i]*x, dp[i-1][1] + v[i]*x);
    dp[i][1] = max(dp[i][1], v[i]*x);

    // if I don't want to multiply things anymore
    dp[i][2] = max({dp[i-1][2] + v[i], dp[i-1][1] + v[i], v[i]});
