...
auto main() -> signed {
  ...
  if(car == 'A') {
    out(a + max({turnBA(b) + turnCA(c), turnBA(b + turnCB(c)), turnCA(c + turnBC(b))}));
  }else if(car == 'B') {
    out(b + max({turnCB(c) + turnAB(a), turnCB(c + turnAC(a)), turnAB(a + turnCA(c))}));
  }else {
    out(c + max({turnAC(a) + turnBC(b), turnAC(a + turnBA(b)), turnBC(b + turnAB(a))}));
  }
}
