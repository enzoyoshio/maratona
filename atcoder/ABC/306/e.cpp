...
auto main() -> signed {
  ...
  while(q--) {
    ...
    if(sz(maiores) < k) {
      ...
      while(resto.size() && maiores.size() < k) {
        maiores.insert(*rbegin(resto));
        idxs.insert(rbegin(resto)->second);
        sum += rbegin(resto)->first;
        resto.erase(prev(end(resto)));
      }

      while(resto.size() && maiores.size() && begin(maiores)->first < rbegin(resto)->first) {
        idxs.erase(begin(maiores)->second);
        resto.insert(*begin(maiores));
        sum -= begin(maiores)->first;
        db("tirando ", var(begin(maiores)->first));
        maiores.erase(begin(maiores));

        maiores.insert(*prev(end(resto)));
        idxs.insert(rbegin(resto)->second);
        sum += rbegin(resto)->first;
        db("adicionando ", var(rbegin(resto)->first));
        resto.erase(prev(end(resto)));
      }
    }
    db(var(maiores));
