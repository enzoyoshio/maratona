...
auto main() -> signed {
  ...
  in(n, m);
  V<int> dig(m);
  V<int> cost = {0,2,5,5,4,5,6,3,7,6};
  in(dig);
  sort(rall(dig));

  // dp[i] = {number used in the i-th state, length of current number}
  V<ii> dp(n+8, ii{0, 0});

  for(int i = 1; i <= n; i++) {

    // at first, we dont use any number and threfore their length is 0
    dp[i] = {0, -1};

    for(auto w: dig) {
      if(i - cost[w] >= 0 && dp[i-cost[w]].second+1 > dp[i].second) {
        auto [dig1, len1] = dp[i];
        auto [dig2, len2] = dp[i-cost[w]];
        dp[i] = ii{w, len2+1};
      }
    }
