...
int f(int diag, int qts) {
  if(diag >= n+m-1) {
    return 1;
  }
//  db(var(diag), var(qts));
  int ans = f(diag+1, 
  for(int b = qts; b <= total[diag]; b++) {
    db(var(diag), var(qts), var(b));
    ans = add(ans, f(diag+1, max(qts - (diag > m ? 1 : 0), qt[diag+1])));
  }
  return ans;
}
