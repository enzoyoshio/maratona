...
struct kbig {
  ...
  multiset<int> ks, rest;
  int k, sum;

  kbig(int k) : k(k), sum(0) {}
  ...
  void erase(int el) {
    if(rest.find(el) != end(rest)) {
      rest.erase(rest.find(el));
    }else
      ks.erase(ks.find(el)), sum -= el;
    balancear();
  }
  ...
  void balancear() {
    ...
    while(ks.size() < k && rest.size()) {
      auto el = *rbegin(rest);
      sum += el;
      ks.insert(el);
      rest.erase(prev(end(rest)));
    }
  }
};
...
auto main() -> signed {
  ...
  for(int i = n; i < 2*n; i++) {
    ans = max(ans, left.sum + right.sum);
    left.add(v[i]);
    right.erase(-v[i]);
  }
  ans = max(ans, left.sum + right.sum);
