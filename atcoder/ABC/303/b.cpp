...
auto main() -> signed {
  ...
  int n, m;
  in(n, m);
  V<V<int>> grid(m, V<int>(n));
  in(grid);

  V<V<int>> friends(m, V<int>(n));
  ...
  for(auto line: grid) {
    for(int i = 0; i+1 < line.size(); i++) {
      auto p1 = no.find({line[i], line[i+1]});
      if(p1 != end(no)) no.erase(p1);
      auto p2 = no.find({line[i+1], line[i]});
      if(p2 != end(no)) no.erase(p2);
    }
  }
  out(no.size()/2);
}
