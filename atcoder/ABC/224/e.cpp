...
auto main() -> signed {
  ...
  int h, w, n;
  in(h, w, n);
  //map<int, V<iiii>> value;
  V<iiii> reading;
  for(int i = 0; i < n; i++) {
    int r, c, a; in(r, c, a);
    //value[-a].eb(iiii{a, r, c, i});
    reading.eb(iiii{a, r, c, i});
  }
  sort(rall(reading));
  V<V<iiii>> value;

  for(auto [a, r, c, i]: reading) {
    if(value.empty() || value.back()[0][0] != a) {
      value.pb({iiii{a, r, c, i}});
    }else value.back().eb(iiii{a, r, c, i});
  }

  V<int> ans(n);
  vector<int> xs(h+1), ys(w+1);
  for(auto cur_value: value) {
    for(auto [a, r, c, idx]: cur_value) {
      ans[idx] = max(xs[r]+1, ys[c]+1);
    }

    for(auto [a, r, c, idx]: cur_value) {
      xs[r] = max(xs[r], ans[idx]);
      ys[c] = max(ys[c], ans[idx]); 
    }
  }
