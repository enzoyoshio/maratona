...
auto main() -> signed {
  ...
  int deli = 0;
  for(int i = 0; i < k; i++) {
    if(freq.find(v[i].second) == end(freq))
      bigger.emplace(v[i].first);
    else
      rest.emplace(v[i].first);

    deli += v[i].first;
    freq[v[i].second]++;
  }

  // answer if I take K biggest deliciousness 
  ans = deli + sz(freq) * sz(freq);

  for(int i = k; i < n; i++) {
    if(freq.find(v[i].second) == end(freq) && rest.size()) {
      deli -= *begin(rest);
      rest.erase(begin(rest));

      deli += v[i].first;
      bigger.insert(v[i].first);
      freq[v[i].second]++; 

      ans = max(ans, deli + sz(freq) * sz(freq));
    }
  }
  out(ans);
}
