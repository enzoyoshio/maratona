...
auto main() -> signed {
  ...
  // tenho que checar para os intervalos de tamanho k
  if(m == 1) {
   ...
   int ans = 0;
    for(int i = 0; i+k-1 < posX.size(); i++) {
      ans = max(ans, (i+k == posX.size() ? (int)posX.size() : posX[i+k]) - (i == 0 ? 0 : posX[i-1]+1));
    }
    out(ans);
    return 0;
  }
  ...
  db(var(pega));
  db(var(k/qt*n));
  for(int left = 0; left <= pega; left++) {
    int right = pega - left;

    db(var(left), var(right));
    db(var(posX[left]), var(reversed[right]));
    ans = max(ans, posX[left] + reversed[right]);
  }
  out(ans + (k/qt) * n);
}
