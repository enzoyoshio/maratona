...
node dfs(int u, int p) {
  ...
  node cur;
  cur.m[cor[u]] = 1;
  cur.sum = cur.m[cor[u]] * (total[cor[u]] - cur.m[cor[u]]);

  for(auto v: g[u]) if(v != p) {
    
    auto m = dfs(v, u);
    if(m.m.size() > cur.m.size()) swap(m, cur);
    for(auto [cc, freq]: m.m) { 

      cur.sum -= cur.m[cc] * (total[cc] - cur.m[cc]);
      cur.m[cc] += freq;
      cur.sum += cur.m[cc] * (total[cc] - cur.m[cc]);
    }
  }

  ans[edges[{u, p}]] = cur.sum;
  return cur;
}
...
auto main() -> signed {
  ...
  ans.resize(n);
  total.resize(n);
  for(auto& it: cor) cin >> it, it--, total[it]++;
