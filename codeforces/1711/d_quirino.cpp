...
auto main() -> signed {
  ...
  int t; in(t); while (t--) {
    ...
    min_priority_queue<pair<int, Type>> events;

    for (auto [x, p] : rain) {
      events.push({x-p, Left});
      events.push({x, Center});
      events.push({x+p, Right});
    }
    ...
    int delta = 0;
    int val = 0;
    int last_x = events.top().ff;
    while (!events.empty()) {
      auto [x, type] = events.top();

      int len = x - last_x;
      last_x = x;
      val += delta * len;

      while (!events.empty() && events.top().ff == x) {
        auto [x, type] = events.top(); events.pop();
        delta += type;
      }

      if (type == Center && val > m) {
        int over = val - m;
        miq(max_l, x-over);
        maq(min_r, x+over);
      }
...
// lemma: maximum is always one of the points with rain
