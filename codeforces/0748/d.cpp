...
auto main() -> signed {
  ...
  map<pair<vector<int>,vector<int>>, vector<int>> ans;
  ...
  for(int i = 0; i < n; i++) {
    ...
    */
    ...
    auto forward = h.query(0, m-1);
    auto rev = r.query(0, m-1);
    auto p = pair<vector<int>,vector<int>>(forward, rev);
    ans[p].emplace_back(val);
  }
  ...
  for(auto [a, b]: ans) {
    ...
    auto aa = pair<vector<int>,vector<int>>(a.second, a.first);
    if(ans.find(aa) == ans.end()) continue;
    auto b2 = ans[aa];
    sort(begin(b), end(b));
