...
auto main() -> signed {
  ...
  int t; in(t); while(t--) {
    int n; in(n);
    V<int> a(n), b(n), mini(n), maxi(n);
    in(a, b);

    int menor = 0;
    // i hope this dont give me rte
    // actually, i hope this logic is right
    for(int i = 0; i < n; i++) {
      while(a[i] > b[menor]) menor++;
      mini[i] = b[menor] - a[i];
    }
    ...
    for(int i = n-1; i >= 0; i--) {
      while(i != maior && maior-1 >= 0 && b[maior-1] < a[maior]) maior--;
      if(i+1 < n && b[i] < a[i+1]) maior = i;
      maxi[i] = b[maior] - a[i];
    }
