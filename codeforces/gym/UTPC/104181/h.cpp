...
auto main() -> signed {
  ...
  in(n, m);
  V<iiii> ret(n);
  for(auto& [a, b, c, d]: ret) in(a, b, c, d);

  V<int> cols(m);
  for(auto& a: cols) in(a);
  sort(begin(cols), end(cols));

  int ans = 0;
  for(auto [r1, c1, r2, c2]: ret) {
//    db(var(r1), var(c1), var(r2), var(c2));
    ans += (abs(r1-r2)+1) * (abs(c1-c2)+1);
 //   db(var((abs(r1-r2)+1) * (abs(c1-c2)+1)));

    auto l1 = lower_bound(begin(cols), end(cols), c1);
    auto l2 = upper_bound(begin(cols), end(cols), c2);
    ans -= (int)abs(l1-l2) * (abs(r1-r2)+1);
  //  db(var(abs(l1-l2) * (abs(r1-r2)+1)));
  }
  out(ans);
}
